...
filterList.push({ key: 0, name: 'filter-value-@', value: '@', includes: true }, { key: 1, name: 'filter-value-com', value: 'com', includes: true }, { key: 2, name: 'filter-value-r', value: '禪', includes: false });
addressList.push['martin.lohrer@gmail.com', 'martin.lohrer@googlemail.com']
    ...
    try {
        ...
        filterList
            .forEach(item => {
                ...
                form.classList.add('filter-form', 'flex', 'flex-col', 'sm:flex-row', 'sm:space-x-6', 'space-y-2', 'sm:space-y-0', 'w-full', 'sm:w-2/3');
        ...
        const filterIncludesDomElement = document.getElementsByClassName('filter-includes');
...
document.getElementById('email-form').addEventListener('submit', event => event.preventDefault());
document.getElementById('email-input').addEventListener('keydown', event => event.key === 'Enter' ? event.preventDefault() : '');
